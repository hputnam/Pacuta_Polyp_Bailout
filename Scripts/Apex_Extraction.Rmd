...
Read in xml info
```{r}
apex.xmlfile <- xmlParse("http://10.10.204.74/cgi-bin/datalog.xml?sdate=250814&days=1.5") #read in the date (e.g. 180620) plus # days (e.g. days=4) of Apex data
...
Salinitys<- ggplot(data_long, aes(x = Time_Hours, y = Salinity, color = Chamber)) +
  geom_point() +
...
print(Salinitys)
ggsave(
  filename = paste0("Outputs/apex_Chamber_Salinity_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".pdf"),
  plot = Salinitys,
  height = 6, width = 8)
``` 
