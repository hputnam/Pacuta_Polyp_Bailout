...
Read in xml info
```{r}
apex.xmlfile <- xmlParse("http://10.10.204.74/cgi-bin/datalog.xml?sdate=250719&days=2") #read in the date (e.g. 180620) plus # days (e.g. days=4) of Apex data
...
apex_pull_date <- "250719"

write.csv(Apex.Data.apex, paste0("Data/",apex_pull_date,"_","Apex.Data.csv")) #write file to save data
...
# Conductivity Probe Cross Calibration Probe_1 to Probe_2
```{r}
...
coeffs <- coef(Ref_c_2)
r_squared <- summary(Ref_c_2)$r.squared
...
#Correcting Probe 2 to the reference Probe 1 for conductivity probes
```{r}
...
Apex.Data.apex$test_cond_2 <- Apex.Data.apex$P_Cond_2_Corr - Apex.Data.apex$P_Cond_1
mean(Apex.Data.apex$test_cond_2)
...
ggplot(Apex.Data.apex, aes(x = P_Cond_1, y = P_Cond_2_Corr)) +
  ...
  #annotate("text", x = 2, y = 60, label = eq_label, hjust = 0, size = 5)+
  labs(
    title = "Linear Regression: P_Cond_1 vs P_Cond_2_Corr",
    x = "P_Cond_1",
    y = "P_Cond_2_Corr"
  ) +
  theme_minimal()
...
# Conductivity Probe Cross Calibration Probe_1 to Probe_3
```{r}

str(Apex.Data.apex)

Apex.Data.apex$P_Cond_1 <- as.numeric(Apex.Data.apex$P_Cond_1)
Apex.Data.apex$P_Cond_3 <- as.numeric(Apex.Data.apex$P_Cond_3)

Ref_c_3 <- Apex.Data.apex %>%
  lm(P_Cond_1 ~ P_Cond_3, data = .)
...
coeffs <- coef(Ref_c_3)
r_squared <- summary(Ref_c_3)$r.squared

eq_label <- sprintf("y = %.2f + %.2fx, R² = %.3f", coeffs[1], coeffs[2], r_squared)


ggplot(Apex.Data.apex, aes(x = P_Cond_3, y = P_Cond_1)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  #annotate("text", x = 2, y = 60, label = eq_label, hjust = 0, size = 5)+
    geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  ...
  labs(
    title = "Linear Regression: P_Cond_3 vs P_Cond_1",
    x = "P_Cond_3",
    y = "P_Cond_1"
  ) +
  theme_minimal()
...
#Correcting Probe 3 to the reference Probe 1 for conductivity probes
```{r}
...
#correct the probe for the relationship (y=mx+b) with the reference probe
Apex.Data.apex$P_Cond_3_Corr <- coeffs[1]+(coeffs[2]*Apex.Data.apex$P_Cond_3)

Apex.Data.apex$test_cond_3 <- Apex.Data.apex$P_Cond_3_Corr - Apex.Data.apex$P_Cond_1
mean(Apex.Data.apex$test_cond_3)

ggplot(Apex.Data.apex, aes(x = P_Cond_1, y = P_Cond_3_Corr)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  #annotate("text", x = 2, y = 60, label = eq_label, hjust = 0, size = 5)+
  labs(
    title = "Linear Regression: P_Cond_1 vs P_Cond_3_Corr",
    x = "P_Cond_1",
    y = "P_Cond_3_Corr"
  ) +
  theme_minimal()
...
# Temperature Probe Cross Calibration 2 to 1
```{r}
str(Apex.Data.apex)

Apex.Data.apex$P_Tmp_1 <- as.numeric(Apex.Data.apex$P_Tmp_1)
Apex.Data.apex$P_Tmp_2 <- as.numeric(Apex.Data.apex$P_Tmp_2)

Ref_t_2 <- Apex.Data.apex %>%
  lm(P_Tmp_1 ~ P_Tmp_2, data = .)
...
coeffs <- coef(Ref_t_2)
r_squared <- summary(Ref_t_2)$r.squared

eq_label <- sprintf("y = %.2f + %.2fx, R² = %.3f", coeffs[1], coeffs[2], r_squared)


ggplot(Apex.Data.apex, aes(x = P_Tmp_2, y = P_Tmp_1)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  #annotate("text", x = 2, y = 60, label = eq_label, hjust = 0, size = 5)+
    geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  ...
  labs(
    title = "Linear Regression: P_Tmp_2 vs P_Tmp_1",
    x = "P_Tmp_2",
    y = "P_Tmp_1"
  ) +
  theme_minimal()
...
#Correcting Temp Probe 2 to Reference Temp Probe 1
```{r}

#P_Tmp_1 = Reference probe
#y=2.31+0.917x

#correct the probe for the relationship (y=mx+b) with the reference probe
Apex.Data.apex$P_Tmp_2_Corr <- coeffs[1]+(coeffs[2]*Apex.Data.apex$P_Tmp_2)

Apex.Data.apex$test_Tmp_2 <- Apex.Data.apex$P_Tmp_2_Corr - Apex.Data.apex$P_Tmp_1
mean(Apex.Data.apex$test_Tmp_2)

ggplot(Apex.Data.apex, aes(x = P_Tmp_1, y = P_Tmp_2_Corr)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  #annotate("text", x = 2, y = 60, label = eq_label, hjust = 0, size = 5)+
  labs(
    title = "Linear Regression: P_Tmp_2 vs P_Tmp_2_Corr",
    x = "P_Tmp_1",
    y = "P_Tmp_2_Corr"
  ) +
  theme_minimal()
