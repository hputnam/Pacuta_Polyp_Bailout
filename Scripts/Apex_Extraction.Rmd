...
Read in xml info
```{r}
apex.xmlfile <- xmlParse("http://10.10.204.74/cgi-bin/datalog.xml?sdate=250711&days=3") #read in the date (e.g. 180620) plus # days (e.g. days=4) of Apex data
...
Apex.Data.apex <- Apex.Data %>%
  select(c(date, probe.value, probe.value.1, probe.value.2, probe.value.3, probe.value.4, probe.value.5))

#rename columns
colnames(Apex.Data.apex) <- c("date", "P_Tmp_1", "P_Cond_1", "P_Tmp_2", "P_Cond_2", "P_pH_1", "P_pH_2")
...
# Conductivity Probe Cross Calibration
```{r}
...
# Temperature Probe Cross Calibration
```{r}
str(Apex.Data.apex)
